...
# is rendered. The endpoint of the parabola is offset in the radial direction
# by a distance controlled by the "parabola range" slider.
# -------------------------------
def draw_camera_feed(dome_radius, parabola_range):
    ...
    # The endpoint is computed by moving in the radial direction from the dot by 'parabola_range'
    # and the control point is taken as the midpoint plus an upward offset.
    ...
    for i in range(20):
        for j in range(20):
            ...
            dir_x = dot_x / mag
            dir_z = dot_z / mag
            # Starting point p0 is at the dot.
            p0 = (dot_x, 0, dot_z)
            # p_end is offset radially outward by 'parabola_range'
            p_end = (dot_x + dir_x * parabola_range, 0, dot_z + dir_z * parabola_range)
            # Control point is set to be the midpoint with an upward offset (H is chosen as half the parabola_range)
            H = 0.5 * parabola_range
            p_control = ((p0[0] + p_end[0]) / 2.0, H, (p0[2] + p_end[2]) / 2.0)
...
# A new parameter 'parabola_range' (controlled via a slider) is passed to the camera feed drawing function.
# -------------------------------
def render_scene(dome_radius, grid, parabola_range):
    ...
    draw_camera_feed(dome_radius, parabola_range)
    glDisable(GL_DEPTH_TEST)
    draw_dome_square_patches(dome_radius, grid)
    glEnable(GL_DEPTH_TEST)
    glPopMatrix()
    
    # Optionally draw a red dot at the apex of the dome for reference.
    glPushMatrix()
    glColor3f(1, 0, 0)
    glPointSize(10)
    glBegin(GL_POINTS)
...
# and use sliders to adjust dome size, grid resolution, and parabola range.
...
def main():
    ...
    pygame.display.set_caption("Dome with Square Patches, Parabolic Trajectories, and Adjustable Sliders")
    glEnable(GL_DEPTH_TEST)
    ...
    # Sliders for dome radius, grid resolution, and parabola range (trajectory distance)
    dome_slider = Slider(10, 10, 200, 20, 100, 400, 300)
    grid_slider = Slider(10, 40, 200, 20, 5, 80, 20)  # Grid resolution (number of patches)
    parabola_slider = Slider(10, 70, 200, 20, 50, 300, 100)  # Controls the range of the parabolic trajectories
    ...
    while running:
            ...
            if event.type == MOUSEBUTTONDOWN:
                if event.button == 1:
                    ...
                    if (not dome_slider.rect.collidepoint(event.pos) and
                        ...
                        not parabola_slider.rect.collidepoint(event.pos)):
                        rotating = True
        ...
        render_scene(dome_radius, grid, parabola_range)
        pygame.display.flip()
