...
let exportCSVButton;  // new export button for CSV (second set)
let patchExtensionFactors2 = []; // per-patch extension factor for set 2
...
// We'll also use a per-patch array for the first set if needed (here we use the global slider for drawing)
// For the second set we use patchExtensionFactors2 (size 20x20)
// Also, when a button is clicked, we record the selected patch indices:
...
function setup() {
  ...
  for (let i = 0; i < cameraGridCount; i++) {
    ...
    for (let j = 0; j < cameraGridCount; j++) {
      ...
      // Compute absolute position for the button in the document
...
function drawDomeWithScreen(radius, grid) {
  ...
  // Draw dome patches with fountain arcs (both sets)
...
function drawDome(radius, grid) {
  ...
  for (let i = 0; i < grid; i++) {
    for (let j = 0; j < grid; j++) {
      ...
      // Use the per-patch factor for set 2 (default 1.2 if not adjusted)
...
// First fountain arc: inner segment blinks in laser red (slow) and outer segment is bright green.
...
// Second fountain arc: inner segment blinks in pink (fast blink) and outer segment in laser red (faster blink).
function drawFountainArc2(patchCenter, extFactor2) {
  ...
  // Outer segment: blinking laser-shine red (even faster blink: period 10 frames)
  if (frameCount % 10 < 5) {
    stroke(255, 0, 0);
    noFill();
    beginShape();
...
// update both sliders to show the stored extension factors,
// and visually highlight the selected button.
function selectCameraPatch(row, col) {
  selectedCameraRow = row;
  selectedCameraCol = col;
  // Update sliders to reflect the stored values
  trajectorySlider.value(globalExtensionFactor); // first set uses global slider
  trajectorySlider.show();
...
function exportCSV() {
  ...
  let extendedEndpoint = p5.Vector.mult(patchCenter, extFactor2);
  
  let x_poly = nf(extendedEndpoint.x, 1, 2) + " * t";
  let y_poly = nf(extendedEndpoint.y, 1, 2) + " * t";
  let z_poly = "(" + nf(extendedEndpoint.z + 4 * arcHeight, 1, 2) + ") * t - " + nf(4 * arcHeight, 1, 2) + " * t^2";
