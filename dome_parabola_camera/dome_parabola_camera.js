...
function drawDomeWithScreen(radius, grid) {
  ...
  // Draw the dome patches with overlays
...
function drawDome(radius, grid) {
  ...
  for (let i = 0; i < grid; i++) {
    for (let j = 0; j < grid; j++) {
      // Map grid indices to angles for the dome surface
      ...
      // Compute the four vertices for the current patch
      ...
      // Draw the patch (a quadrilateral)
      beginShape();
      ...
      // Draw a parabolic fountain arc from the dome center (0,0,0) to the patch center
      drawFountainArc(center);
    }
  }
}

// Draws a parabolic (fountain-like) arc from the origin to a given end point.
// The arc adds an upward offset (in z) that peaks at t=0.5.
function drawFountainArc(endPoint) {
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    let P = p5.Vector.lerp(start, endPoint, t);
    // Add a parabolic offset in the z direction.
    // 4*t*(1-t) is 0 at t=0 and t=1 and peaks at t=0.5.
    let arcHeight = 50; // adjust this value to change the fountain curve's height
