...
let topRotation = 0; // not used now but kept for future interactivity
...
function setup() {
  ...
  // Updated slider range: 1.0 to 10.0 with initial value 5.0
  ...
  // Updated slider range for set 2 as well: 1.0 to 10.0 with initial value 5.0
  ...
  for (let i = 0; i < cameraGridCount; i++) {
    ...
    for (let j = 0; j < cameraGridCount; j++) {
      // Initialize each patch's extension factor for set 2 with default value 5.0
...
function draw() {
  ...
  push();
  // Center the dome in the canvas and rotate it so the dome base is horizontal.
  // Rotate around X so that the dome base (theta ≈ HALF_PI) points upward.
  translate(0, 0, 0);
  rotateX(-PI/2);
...
function drawDomeWithScreen(radius, grid) {
  // Draw a video-textured plane behind the dome (optional background)
...
function drawDome(radius, grid) {
  ...
  // Loop over the dome grid (assumed similar to the camera grid)
...
// First fountain arc (Set 1): inner segment blinks in slow laser red; outer segment is bright green.
...
// (The exported polynomial for Set 2 is computed from the dome patch's "base" as before.)
...
function mouseDragged() {
  let dx = movedX * 0.01;
  let dy = movedY * 0.01;
  // (Mouse interactivity for rotation can be added if desired)
}
