...
function setup() {
  ...
  // Set up the single dome view (we use topView, though only one dome is drawn)
...
function draw() {
  ...
  push();
  // Translate downward so the dome base (z=0) lies at ground level.
  // First, rotate the dome: rotateX(PI/2) maps (x,y,z) -> (x, -z, y).
  // Then, translate downward by domeSize/2 so that the base (which was at y = 0) becomes at the bottom.
...
function drawDomeWithScreen(radius, grid) {
  ...
  // Draw dome patches and both fountain arc sets.
...
function drawDome(radius, grid) {
  ...
  for (let i = 0; i < grid; i++) {
    for (let j = 0; j < grid; j++) {
      ...
      // Compute the patch center (average of vertices).
      ...
      // Map dome patch indices (i,j) to camera grid indices (assumed 20×20).
...
// (The exported polynomial for Set 2 is computed from the dome patch's "base".)
...
// Helper function: Convert spherical coordinates to Cartesian.
...
function mouseDragged() {
  // (Optional interactivity for rotation can be added)
}
...
// The polynomial is computed from the "base" of the dome patch. For the selected patch,
// we compute the base point using the lower boundary (using theta2).
function exportCSV() {
  if (selectedCameraRow == -1 || selectedCameraCol == -1) {
  ...
  let domeCol = floor(selectedCameraCol * (G / cameraGridCount));
  let phi = map(domeCol + 0.5, 0, G, 0, TWO_PI);
  // Compute baseTheta using the lower boundary of the patch.
  let baseTheta = map(domeRow + 1, 0, G, 0, HALF_PI);
  let basePoint = spherePoint(domeSize, baseTheta, phi);
  // Also compute the patch center (midpoint theta)
